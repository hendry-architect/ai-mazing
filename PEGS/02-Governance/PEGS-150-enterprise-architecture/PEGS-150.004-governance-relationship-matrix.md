# PEGS-150.004 — Governance Relationship Matrix
...
> **Extends PEGS-100/101.** One row-set per entity. 🔶/TO CONFIRM cells
> await the Founder's answers (Missing Information Report); TARGET marks
> the post-Phase-6 state. This matrix is the enterprise-level view; each
> entity's operational detail lives in its own authority matrix (L02
> template) once instantiated.
...
| Body | Decides | Approval rule | Escalation path | Frequency |
|---|---|---|---|---|
| Founder | Class 1 + Class 2 (PEGS-000 Art. V) | Sole, written | — (constitutional amendments: Art. VIII process) | Continuous |
| Executive leaders | Class 3 within role charters | Single accountable holder | Class 2 trigger → Founder memo | Weekly/monthly rhythm |
| Strategic Advisory Council 🔮 | Nothing (advises) | N/A — advice recorded, attributed | Advice → Founder decision, logged | Quarterly + retreat |
| Stewardship Council (dormant) | Class 1 on activation only | 2/3 supermajority constitutional; simple majority otherwise (PEGS-101 §5) | Immutable Core beyond reach | Annual review pre-activation |
| Foundation Board 🔮 | Foundation governance | Majority; conflicts recused (L08) | Grants per SOP thresholds | Quarterly |
| Trustee / Legacy Council 🔮 | Trust administration | Per instrument (drafted with counsel) | Per instrument | Annual minimum |

## 4. Escalation procedures (uniform, all entities)

1. Role holder hits a Class 2 trigger → stop, write the decision memo →
   Founder (or chartered authority).
2. Firewall touch (clinical/legal/PHI, brand, compliance) → escalate
   immediately, no thresholds apply (PEGS-000 Art. VI §4).
3. Unclear class → treat as one class higher, escalate once (Art. V §1).
4. Cross-entity conflict → Holdings coordinates (TARGET); Founder until then.
5. Anything touching the Immutable Core → automatic fail (Art. VIII §2).

## Governance notes

No cell in this matrix grants authority by itself — authority flows from
ratified charters and matrices (L02); this document is the map of where
authority *lives*, not a source of it.

## Implementation recommendations

1. Resolve every TO CONFIRM cell via the Missing Information Report before
   Phase 4 instantiation.
2. When each entity's own authority matrix instantiates (Phase 5-old/now-
   Phase 4 wave), verify it against its row here; mismatch = stop and
   reconcile.

## Future dependencies

PEGS-100 amendment · entity authority matrices · trust instrument (Phase 6)
· SAC seating · Foundation board formation.
