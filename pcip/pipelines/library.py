    ...
    if attached:
        ...
        raise HandoffRequired("export", {
            ...
            "how": (
                ...
                "Also make a proper landscape version for the website: duplicate "
                "the design (MCP: copy-design), then rebuild it as a landscape "
                "layout — full-bleed photo cropped to fill the new frame, the "
                "logo kept, the navy band and copy re-laid to fit. Found live: "
                "resize-design (type=custom, e.g. 1200x750) does NOT reflow a "
                "design onto its new dimensions — it letterboxes, leaving white "
                "margins and an orphaned CTA outside the visible frame. Don't "
                "use its output as-is; treat this as a small redesign for a new "
                "aspect ratio, not a resize. Export the rebuilt version, and "
                "attach it separately in the SAME attach call as the exports "
                "above (this is required — attaching it in a later, separate "
                "call is accepted with no error and silently does nothing, "
                "because nothing reads the run's context again once this step "
                "has completed):\n"
                f"  pcip attach {ctx['run'].id} --export-url <url> "
                "--featured-image-file <path>\n"
                "This step is optional — skipping it just means the portrait "
                "export is used as the featured image as before, with the crop "
                "it already has."
            ),
        })
    else:
